...
2. Determine the intent:
   - **Create**: assign the next id (t1, t2, …) and run `new-task.sh <id> "<title>"` — it adds the registry row, scaffolds `.proc/tasks/<id>/task.md`, and makes the task ACTIVE in `intake`.
...
## Transition
- Created a task → `new-task.sh <id> "<title>"` (row + `tasks/<id>/task.md` + ACTIVE in one step); then fill the task's definition in `tasks/<id>/task.md` during `intake`.
